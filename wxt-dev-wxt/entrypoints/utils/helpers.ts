...
export async function waitForPageLoad() {
    // FIX: was "if (!isDocumentReady)" which compared the function reference (always truthy).
    // Corrected to invoke the function: isDocumentReady()
    if (!isDocumentReady()) {
        await new Promise<void>(resolve => {
...
/**
 * Sends a browser push notification when a game is successfully claimed.
 * Requires the "notifications" permission in the manifest (already present).
 */
export function sendClaimNotification(title: string, platform: string) {
    try {
        browser.notifications.create(`claim-${Date.now()}`, {
            type: 'basic',
            iconUrl: browser.runtime.getURL('/icon/128.png'),
            title: '🎮 Game Claimed!',
            message: `"${title}" has been added to your ${platform} library.`,
        });
    } catch (e) {
...
/**
 * Sends a browser push notification when new free games are detected
 * (before claiming — so the user knows games are available even if auto-claim is off).
 */
export function sendNewGamesNotification(count: number) {
    try {
        const plural = count === 1 ? 'game' : 'games';
        browser.notifications.create(`new-games-${Date.now()}`, {
            type: 'basic',
            iconUrl: browser.runtime.getURL('/icon/128.png'),
