/**
 * amazonScraper.ts — DOM scraping and game detection for Amazon Prime Gaming / Luna.
 *
 * Extracted from amazon.content.ts to improve maintainability.
 * Contains: scrapeAllGameCards, fetchUpcomingGames, helper functions for card parsing.
 */
 ...
 */
export function scrapeAllGameCards(): { claimable: FreeGame[]; upcoming: FreeGame[] } {
 ...
 */
export async function fetchUpcomingGames(): Promise<FreeGame[]> {
    ...
    // Strategy 2: GraphQL persisted query
    const graphqlHashes = [
...
export function detectLogin(): boolean {
